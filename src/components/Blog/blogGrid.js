                        ...
                        : this.props.loading 
                            ...
                            : (
                                <div className="blog__grid__container">
                                    ...
                                    <Link key={this.props.mainPost.id} to={{pathname: `/prepxp/blog/${this.props.mainPost.title.split(" ").join("-")}`, state:{post: this.props.mainPost, posts: this.props.allPosts}}}>
                                        <BlogMainPanel 
                                    ...
                                    <div className="blog__grid">
                                        {this.props.editorPosts.map((post, index) => (
                                            <Link key={post.title + index} to={{pathname: `/prepxp/blog/${post.title.split(" ").join("-")}`, state:{post, posts: this.props.allPosts}}}>
                                    ...
                                    <div className="blog__grid">
                                        {this.props.posts.map((post, index) => (
                                            <Link 
                                                key={post.id} 
                                                to={{
                                                    pathname: `/prepxp/blog/${post.title.split(" ").join("-")}`, state: {post, posts: this.props.allPosts}
                                                    }}>
                                                <BlogPanel post={post} comments={this.props.blog.filter(blog => blog.id === post.id)[0].comments}/>
                                            </Link>
                                        ))}
